...
class PacMan:
    def __init__(self, x, y, radius,map):
        self.x = x
        self.y = y
        self.radius = radius
        self.isOpen = False
        ...
        self.t1.start()
        self.stop = False
        self.step = map.XSizeCell()
        self.big = False
        speed = 25
        self.EndFrightened1 =10/speed
    ...
    def LocalCoordinatesVisualization(self, map, screen):
        v = []
        x = int((self.x)/(map.XSizeCell())) 
        y = int((self.y)/(map.YSizeCell()))
        v.append(x)
        v.append(y)
        pygame.draw.rect(screen, RED, (map.XSizeCell()*(x), map.YSizeCell()*(y), map.XSizeCell(), map.YSizeCell()))
        return v

    def IntDirection(self):
        ...
        if(self.direction=="TOP"):
            return 2
        if(self.direction=="BOT"):
            return 3
        return 0

    def EndsFrightened(self, rg, pg, bg, og):
        time.sleep(self.EndFrightened1)
    ...
    def CanPass1(self, direction, Map, screen):
        size = 10
        if(direction == "TOP"):
            x1 = int((self.x)/(Map.XSizeCell()))
            x2 = int((self.x+size)/(Map.XSizeCell())) 
            x3 = int((self.x-size)/(Map.XSizeCell()))
            y1 = int((self.y+size +self.step)/(Map.YSizeCell())) 
            if(x1>=25 or Map.matrix[y1-1, x1] == 1 or x2>=25 or Map.matrix[y1-1, x2] == 1 or x3>=25 or Map.matrix[y1-1, x3] == 1):
                pygame.draw.circle(screen, YELLOW, (Map.XSizeCell()*(x1+0.5), Map.YSizeCell()*(y1-1+0.5)), 10)
                pygame.draw.circle(screen, BLACK, (Map.XSizeCell()*(x1+0.5), Map.YSizeCell()*(y1-1+0.5)), 7)
                return False
            else:
                return True
        elif (direction == "BOT"):
            ...
            x1 = int((self.x-size-self.step)/(Map.XSizeCell())) 
            ...
            if(x1+1<25 and (Map.matrix[y1, x1+1] == 1 or Map.matrix[y2, x1+1] == 1 or Map.matrix[y3, x1+1] == 1)):
                pygame.draw.circle(screen, YELLOW, (Map.XSizeCell()*(x1+1+0.5), Map.YSizeCell()*(y1+0.5)), 10)
                pygame.draw.circle(screen, BLACK, (Map.XSizeCell()*(x1+1+0.5), Map.YSizeCell()*(y1+0.5)), 7)
                return False
            else:
                return True
        elif (direction == "LEFT"):
    ...
    def CanPass2(self, direction, Map):
        size = 10
        if(direction == "TOP"):
            x1 = int((self.x)/(Map.XSizeCell()))
            x2 = int((self.x+size)/(Map.XSizeCell())) 
    ...
    def RunAI(self, rg, pg, bg, og, Map,eat):
        if(self.CanPass2(self.direction, Map)):
            if(self.direction == "TOP"):
                self.y-=self.step
    ...
    def Run(self, direction,rg, pg, bg, og, Map,eat,eatBB, eatG, stop):
        if(not self.stop):
